	...
	GSTextureCache::Target* rt, bool can_scale_rt_alpha, bool& new_rt_alpha_scale)
{
	...
	if (blend_ad_alpha_masked && (((is_basic_blend || (COLCLAMP.CLAMP == 0)) && features.CanSampleFromFB() && features.texture_barrier)
	...
	if (COLCLAMP.CLAMP == 0)
	{
		...
		const bool free_colclip = !has_HDR_texture && (features.framebuffer_fetch || features.raster_order_view || no_prim_overlap || blend_non_recursive);
	...
	if (PABE)
	{
		...
		if (sw_blending)
		{
			...
			else if (features.CanSampleFromFB())
			{
				// PABE sw blend:
